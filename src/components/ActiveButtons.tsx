...
const ActionButtons: React.FC<ActionButtonsProps> = ({ onShoot, onReload }) => {
  ...
  const reloadSound = useRef<HTMLAudioElement>(null);
  const damageSound = useRef<HTMLAudioElement>(null);
  const healSound = useRef<HTMLAudioElement>(null);

  useEffect(() => {
    shootSound.current = new Audio("/shoot.wav");
    reloadSound.current = new Audio("/reload.wav");
    damageSound.current = new Audio("/damage.wav");
    healSound.current = new Audio("/heal.wav");
  }, []);
  ...
  const handleShoot = () => {
    ...
    const opponents = gameState.lobby.players.filter(
      (p) => p.id !== player.id && p.lives > 0
    );
    if (opponents.length === 0) return;

    const randomTarget = opponents[Math.floor(Math.random() * opponents.length)];
    console.log("🔥 Shooting target:", randomTarget.id, randomTarget.color);
  ...
  const handleReload = () => {
    if (currentPlayer.ammo === currentPlayer.currentWeapon.ammoCapacity || currentPlayer.isReloading)
      return;
  ...
  const handlePowerUp = () => {
    if (!currentPlayer.hasPowerUp) return;

    console.log("⚡ Power-up activated!");
    // You can play heal or other sound if needed
    healSound.current?.play();
  };
    ...
    >
      ...
      <button
        ...
        style={{
          ...
          border: "none",
          borderRadius: "15px",
          background: currentPlayer.hasPowerUp ? "linear-gradient(45deg, #ff00ff, #ff0080)" : "rgba(128, 128, 128, 0.3)",
          color: "white",
          fontSize: "1.3rem",
          fontWeight: "bold",
